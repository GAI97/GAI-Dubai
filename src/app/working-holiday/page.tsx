...
export default async function WorkingHolidayIndexPage() {
  const items = await fetchWorkingHolidayVisas()
  return (
    <main className="mx-auto w-full max-w-[1200px] px-4 sm:px-6 lg:px-8 py-12">
      <div className="text-center">
        <div className="text-[#2dc0d9] font-semibold tracking-wide text-sm">WORKING HOLIDAY</div>
        <h1 className="mt-2 text-3xl sm:text-5xl font-semibold text-[#283277]">Choose Your Working Holiday</h1>
      </div>
      {items.length === 0 ? (
        <p className="mt-10 text-center text-sm text-neutral-600">No working holiday items found.</p>
      ) : (
